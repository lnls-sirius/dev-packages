...
class _LL_Base:
    ...
    def _pvs_on_connection(self, pvname, conn, pv):
        _log.debug(self.channel+' PV {0:10s} {1:s}connected'
                   .format(pvname, '' if conn else 'dis'))
        ...
        for ll_prop, pv in self._pvs_sp.items():
            if pv.connected and self._pvs_sp_canput[pv.pvname]:
                ...
                for hl_prop, val in props.items():
                    ...
                    if my_val == val:
                        continue
                    _log.debug(self.channel+' propty = {0:s} Value = {1:s} '
                               .format(hl_prop, str(my_val)))
    ...
    def _pvs_sp_callback(self, pvname, value, **kwargs):
        if value is None:
            _log.debug(self.channel+' pvs_sp_callback; ' +
                       '{0:s} received None'.format(pvname))
            return
        _log.debug(
            self.channel+' pvs_sp_callback; ' +
            'PV = {0:s} New Value = {1:s} '.format(pvname, str(value)))
        if not _FORCE_EQUAL:
            return
        ...
        for hl_prop, val in props.items():
            ...
            if my_val != val:
                _log.debug(
                    self.channel+' pvs_sp_callback; Forcing  ' +
                    'propty = {0:s} Value = {1:s} '.format(hl_prop,
                                                           str(my_val)))
                self.set_propty(hl_prop, my_val)
    ...
    def _set_simple(self, prop, value):
        _log.debug(self.channel +
                   ' propty = {0:s}, value = {1:s}.'.format(prop, str(value)))
...
class _LL_TrigEVRMF(_LL_Base):
    ...
    def _set_delay(self, value):
        ...
        if not pv1.connected:
            _log.debug(self.channel + ' PV ' + pv1.pvname + ' NOT connected.')
        if not pv2.connected:
            _log.debug(self.channel + ' PV ' + pv2.pvname + ' NOT connected.')
        if not pv3.connected:
            _log.debug(self.channel + ' PV ' + pv3.pvname + ' NOT connected.')
...
class _LL_TrigAFCLVE(_LL_TrigEVRMF):
    ...
    def _get_LLPROP_2_PVSP(self):
        ...
        # 'EVGParam-SP'
        map_['event'] = self.prefix + self._INTLB + 'Event-Sel'
        return map_
