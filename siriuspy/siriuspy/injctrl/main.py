...
class App(_Callback):
    ...
    def __init__(self):
        ...
        self._p2w = {
            "Type": {"watcher": None, "status": _Const.IdleRunning.Idle},
            "FilaOpCurr": {"watcher": None, "status": _Const.IdleRunning.Idle},
            "HVOpVolt": {"watcher": None, "status": _Const.IdleRunning.Idle},
            "PUMode": {"watcher": None, "status": _Const.IdleRunning.Idle},
        }
        ...
        for sec in secs:
            ...
            if sec != "LI":
                ...
                self._pvs_diag[sec]["RF"] = {
                    n: _PV(n + ":DiagStatus-Mon", connection_timeout=0.05)
                    for n in _LIDiagConst.DEV_2_LINAME
                    if "RF" in n
                }
            ...
            if sec in ["BO", "SI"]:
                self._pvs_diag[sec]["RF"] = {
                    n: _PV(n + ":DiagStatus-Mon", connection_timeout=0.05)
                    for n in _RFDiagConst.ALL_DEVICES
                    if n.startswith(sec)
                }
        ...
        self._pumode_dev = InjSysPUModeHandler(hltiming=self._hlti_dev)
    ...
    def set_mode(self, value):
        ...
        if (
            self._pumode != _Const.PUMode.Accumulation
            and value == _Const.InjMode.TopUp
        ):
            self._logger.error("Set PUMode to Accumulation before")
            self._logger.error("changing mode to top-up")
            return False

        if value != _Const.InjMode.Decay:
            stg = "top-up" if value == _Const.InjMode.TopUp else "accumulation"
            self._logger.info("Configuring EVG RepeatBucketList...")
            self._evg_dev["RepeatBucketList-SP"] = 1
            self._logger.info("...done. Ready to start %s.", stg)
    ...
    def set_type(self, value):
        ...
        if self._mode != _Const.InjMode.Decay:
            self._logger.error("InjType can only be changed in Decay mode.")
            return False
        if (
            self._p2w["Type"]["watcher"] is not None
            and self._p2w["Type"]["watcher"].is_alive()
        ):
            self._logger.warning("Interrupting type change command..")
        ...
        target = (
            self.egun_dev.cmd_switch_to_single_bunch
            if value == _Const.InjType.SingleBunch
            else self.egun_dev.cmd_switch_to_multi_bunch
        )
    ...
    def set_sglbunbiasvolt(self, value):
        ...
        self._logger.info("Received setpoint to SB Bias voltage.")
        ...
        if self._type == _Const.InjType.SingleBunch:
            _epics.ca.CAThread(
                target=self._set_egunbias, args=[value], daemon=True
            ).start()
        return True

    def set_multbunbiasvolt(self, value):
        """Set multi bunch bias voltage."""
        self._logger.info("Received setpoint to MB Bias voltage.")
    ...
    def _set_egunbias(self, value):
        ...
        self._logger.info("Setting EGun Bias voltage to %.2fV...", value)
        if not self.egun_dev.bias.set_voltage(value, tol=abs(0.005 * value)):
            self._logger.error("Could not set EGun Bias voltage.")
        else:
            self._logger.info("Set EGun Bias voltage: %.2fV.", value)
    ...
    def set_filaopcurr(self, value):
        ...
        if (
            self._p2w["FilaOpCurr"]["watcher"] is not None
            and self._p2w["FilaOpCurr"]["watcher"].is_alive()
        ):
            self._logger.warning("Interrupting FilaPS current ramp...")
    ...
    def set_hvopvolt(self, value):
        ...
        if (
            self._p2w["HVOpVolt"]["watcher"] is not None
            and self._p2w["HVOpVolt"]["watcher"].is_alive()
        ):
            self._logger.warning("Interrupting HVPS voltage ramp...")
    ...
    def set_pumode(self, value):
        ...
        if not 0 <= value < len(_ETypes.PUMODE):
            return False
        if (
            self._mode == _Const.InjMode.TopUp
            and value != _Const.PUMode.Accumulation
        ):
            self._logger.error("In TopUp mode PUMode must be Accumulation.")
            return False
        if (
            self._p2w["PUMode"]["watcher"] is not None
            and self._p2w["PUMode"]["watcher"].is_alive()
        ):
            self._logger.warning("Interrupting PUMode change command")
        ...
        target = (
            self._pumode_dev.cmd_switch_to_accum
            if value == _Const.PUMode.Accumulation
            else self._pumode_dev.cmd_switch_to_optim
            if value == _Const.PUMode.Optimization
            else self._pumode_dev.cmd_switch_to_onaxis
        )
    ...
    def set_target_current(self, value):
        ...
        self._logger.info("Updated target current value: %.2fmA.", value)
        return True
    ...
    def set_bucketlist_step(self, step):
        ...
        if not -_Const.MAX_BKT <= step <= _Const.MAX_BKT:
            return False
        if step == 0:
            self._logger.error("Bucket list step must not be zero.")
            return False
        if self._mode != _Const.InjMode.Decay:
    ...
    def set_isinj_delay(self, value):
        ...
        self._logger.info(
            "Changed IsInjecting-Mon flag delay to %.1fms.", value
        )
    ...
    def set_isinj_duration(self, value):
        ...
        self._logger.info(
            "Changed IsInjecting-Mon flag duration to %.1fms.", value
        )
    ...
    def _cmd_bucketlist_fill(self, stop, start, step):
        ...
        if not self._evg_dev.connected:
            self._logger.error("Could not update bucket list,")
            self._logger.error("EVG is disconnected.")
            return False
        if self._evg_dev.fill_bucketlist(stop, start, step, timeout=3):
            self._logger.info("Updated BucketList.")
            return True
        self._logger.warning("Timed out waiting for BucketList.")
        return False
    ...
    def set_topup_state(self, value):
        ...
        if value == _Const.OffOn.On:
            self._logger.info("Start received!")
            if not self._check_allok_2_inject():
                return False
            if self._topup_job is None or not self._topup_job.is_alive():
                self._launch_topup_job()
        else:
            self._logger.info("Stop received!")
            if self._topup_job is not None and self._topup_job.is_alive():
                self._stop_topup_job()
        return True
    ...
    def set_topup_period(self, value):
        ...
        self._logger.info("Changed top-up period to %smin.", str(value))
    ...
    def set_accum_period(self, value):
        ...
        self._logger.info("Changed accumulation period to %ss.", str(value))
    ...
    def set_topup_headstarttime(self, value):
        ...
        self._logger.info("Changed top-up head start time to %ss.", str(value))
    ...
    def set_topup_pustandbyenbl(self, value):
        ...
        self._logger.info("%sabled PU standby between injections.", text)
    ...
    def set_topup_liwarmupenbl(self, value):
        ...
        text = "En" if value else "Dis"
        self._logger.info("%sabled LI warm up before injections.", text)
    ...
    def set_topup_bopsstandbyenbl(self, value):
        ...
        text = "En" if value else "Dis"
        self._logger.info("abled BO PS standby between injections.", text)
        self.run_callbacks(
            "TopUpBOPSStandbyEnbl-Sts", self._topup_bopsstandbyenbl
        )
        return True
    ...
    def set_topup_borfstandbyenbl(self, value):
        ...
        text = "En" if value else "Dis"
        self._logger.info("abled BO RF standby between injections.", text)
        self.run_callbacks(
    ...
    def set_topup_nrpulses(self, value):
        ...
        if self._mode == _Const.InjMode.TopUp:
            if not self._update_bucket_list():
                return False
        self._logger.info("Changed top-up nr.pulses to %s.", str(value))
    ...
    def cmd_injsys_turn_on(self, value=None, wait_finish=False):
        ...
        if run:
            self._logger.error("Still processing turn %s InjSystem", run)
            return False

        self._logger.info("Sending turn on to Inj.System...")
        self.run_callbacks(
            "InjSysCmdDone-Mon", ",".join(self._injsys_dev.done)
        )
    ...
    def cmd_injsys_turn_off(self, value=None, wait_finish=False):
        ...
        self._logger.info("Sending turn off to Inj.System...")
        self.run_callbacks(
            "InjSysCmdDone-Mon", ",".join(self._injsys_dev.done)
        )
    ...
    def _watch_injsys(self, cmd, timeout=_Const.RF_RMP_TIMEOUT):
        ...
        if is_running:
            self._logger.error("Timed out in turn %s Inj.System.", cmd)
            self._injsys_dev.cmd_abort()
        elif not ret[0]:
            self._logger.error("Failed to turn %s Inj.System.", cmd)
            for msg in ret[1].split("\n"):
                self._logger.error(msg)
            self._logger.error("Detail list: ")
            for item in ret[2]:
                self._logger.error(item)
        else:
            self._logger.info("Turned %s Inj.System.", cmd)
    ...
    def set_injsys_on_order(self, value):
        ...
        if set(new_order) - set(self._injsys_dev.DEF_ON_ORDER):
            self._logger.error("Invalid value for inj.sys. on order")
            return False
        self._injsys_dev.on_order = new_order
        self._logger.info("Updated inj.sys. turn on command order.")
    ...
    def set_injsys_off_order(self, value):
        ...
        if set(new_order) - set(self._injsys_dev.DEF_OFF_ORDER):
            self._logger.error("Invalid value for inj.sys. off order")
            return False
        self._injsys_dev.off_order = new_order
        self._logger.info("Updated inj.sys. turn off command order.")
    ...
    def cmd_rfkillbeam(self, value):
        ...
        if self._rfkillbeam_mon == _Const.RFKillBeamMon.Kill:
            self._logger.error("Still processing RFKillBeam command")
            return False

        self._logger.info("Received RFKillBeam Command...")
    ...
    def _watch_rfkillbeam(self):
        ...
        if not ret[0]:
            ...
            self._logger.info("The beam was killed by RF!")
        self._rfkillbeam_mon = _Const.RFKillBeamMon.Idle
        self.run_callbacks("RFKillBeam-Mon", self._rfkillbeam_mon)
    ...
    def _callback_watch_eguntrig(self, value, **kws):
        ...
        if self._mode != _Const.InjMode.Decay:
            return
        _epics.ca.CAThread(
            target=self._watch_eguntrig, args=[value], daemon=True
        ).start()
    ...
    def _watch_eguntrig(self, value, **kws):
        ...
        while _time.time() - _t0 < 10:
            if self.egun_dev.trigps.is_on() == value:
                self._logger.info("Turned %s EGun.", cmd)
                break
            _time.sleep(0.1)
        else:
            self._logger.warning("Timed out in turn %s Egun.", cmd)
    ...
    def _callback_watch_injectionevt(self, value, **kws):
        ...
        if self._mode != _Const.InjMode.Decay:
            return
        _epics.ca.CAThread(
            target=self._watch_injti, args=[value], daemon=True
        ).start()
    ...
    def _watch_injti(self, value, timeout=_Const.TI_INJ_TIMEOUT):
        ...
        while _time.time() - _t0 < timeout:
            if self._evg_dev.injection_state == value:
                self._logger.info("Turned %s InjectionEvt.", cmd)
                break
            _time.sleep(0.1)
        else:
            self._logger.warning("Timed out in turn %s InjectionEvt.", cmd)

    def _callback_autostop(self, value, **kws):
        _ = kws
        if (
            self._thread_autostop is not None
            and self._thread_autostop.is_alive()
        ):
            return
    ...
    def _thread_run_autostop(self, value, cb_type):
        ...
        if not self.egun_dev.trigps.is_on():
            return
        if cb_type == "cb_val":
            self._logger.info("Target current reached!")
        else:
            self._logger.error("Current PV disconnected.")
        self._run_autostop()

    def _run_autostop(self):
        self._logger.info("Running Auto Stop...")
        if self._stop_injection():
            self._logger.info("Injection Auto Stop done.")
            self._update_bucket_list_autostop()
        else:
            self._logger.error("Injection Auto Stop failed.")
    ...
    def _check_allok_2_inject(self, show_warn=True):
        if show_warn:
            if self._status["All"] != 0:
                self._logger.warning("DiagStatus not ok:")
                for prob in self._status_problems:
                    self._logger.warning("Verify %s!", prob)

            if self._injstatus != 0:
                self._logger.warning("InjStatus not ok:")
                for bit, prob in enumerate(_Const.INJ_STATUS_LABELS):
                    if _get_bit(self._injstatus, bit):
                        self._logger.warning(prob)

        if self._mode != _Const.InjMode.Decay:
            if self._evg_dev.nrpulses != 1:
                self._logger.error("Aborted. RepeatBucketList must be 1.")
                return False

            if self._abort:
                self._logger.info("Abort received.")
                return False
        return True

    def _start_injection(self):
        # turn on injectionevt
        self._logger.info("Turning InjectionEvt on...")
        if not self._evg_dev.cmd_turn_on_injection():
            self._logger.error("Could not turn on InjectionEvt.")
            return False
        self._logger.info("Sent turn on to InjectionEvt.")

        # wait for injectionevt to be ready
        self._logger.info("Waiting for InjectionEvt to be on...")
        _t0 = _time.time()
        while _time.time() - _t0 < _Const.TI_INJ_TIMEOUT:
            ...
            self._logger.error("Timed out waiting for InjectionEvt.")
            return False

        self._logger.info("InjectionEvt is on!")
        return True
    ...
    def _stop_injection(self):
        self._logger.info("Turning off InjectionEvt...")
        if self._evg_dev.cmd_turn_off_injection():
            self._logger.info("Turned off InjectionEvt.")
        else:
            self._logger.error("Failed to turn off InjectionEvt.")
            self._logger.info("Turning off EGun TriggerPS...")
            if self.egun_dev.trigps.cmd_disable_trigger():
                self._logger.info("Turned off EGun TriggerPS.")
            else:
                self._logger.error("Failed to turn off EGun TriggerPS.")
                return False
        return True
    ...
    def _update_bucket_list_autostop(self):
        if not self._evg_dev.connected:
            self._logger.error(
                "Could not update bucket list, EVG is disconnected."
            )
            return False
    ...
    def _set_bucket_list(self, value):
        ...
        while _time.time() - _t0 < 5:
            if _np.all(self._evg_dev.bucketlist == value):
                self._logger.info("Updated BucketList.")
                return True
        self._logger.warning("Could not update BucketList.")
        return False
    ...
    def _launch_accum_job(self):
        while self._abort:
            _time.sleep(0.1)
        self._logger.info("Launching accumulation thread...")
    ...
    def _stop_accum_job(self):
        if self._abort:
            return
        self._logger.info("Stopping accumulation thread...")
        self._abort = True
        self._accum_job.join()
        self._accum_job = None
        self._logger.info("Stopped accumulation thread.")
        self._abort = False
    ...
    def _do_accumulation(self):
        ...
        while self._mode == _Const.InjMode.Accum:
            ...
            self._logger.info("Waiting for next injection...")
        ...
        self._logger.info("Stopped accumulation loop.")
    ...
    def _continue_accum(self):
        if not self.currinfo_dev.connected:
            self._logger.error("CurrInfo device disconnected.")
            return False
        if self.currinfo_dev.current >= self._target_current:
            self._logger.info("Target Current reached. Stopping accumulation.")
            return False
        if not self._check_allok_2_inject():
            return False
        return True
    ...
    def _launch_topup_job(self):
        while self._abort:
            _time.sleep(0.1)
        self._logger.info("Launching top-up thread...")
    ...
    def _stop_topup_job(self):
        if self._abort:
            return
        self._logger.info("Stopping top-up thread...")
        self._abort = True
        self._topup_job.join()
        self._topup_job = None
        self._logger.info("Stopped top-up thread.")
        self._abort = False
    ...
    def _do_topup(self):
        ...
        while self._mode == _Const.InjMode.TopUp:
            ...
            if self.currinfo_dev.current < self._target_current * 1.02:
                ...
                self._logger.info("Skipping injection...")
                _time.sleep(2)
        ...
        self._logger.info("Stopped top-up loop.")
        if not self._abort or self._setting_mode:
    ...
    def _wait_topup_period(self):
        while _time.time() < self._topup_next:
            ...
            if cond and self.currinfo_dev.connected:
                ...
                self._logger.info("BiasFB required InjCurr: %.3fmA", dcur)
        ...
        self._logger.info("Remaining time: 0s")
        return True
    ...
    def _prepare_topup(self):
        ...
        if (
            self._topup_pustandbyenbl
            and self._topup_next - _time.time() > self._topup_puwarmuptime * 2
        ):
            self._handle_aspu_standby_state(standby)

        # LI
        if (
            self._topup_liwarmupenbl
            and self._topup_next - _time.time() > self._topup_liwarmuptime * 2
        ):
            self._handle_liti_warmup_state(standby)

        # BO PS
        if (
            self._topup_bopsstandbyenbl
            and self._topup_next - _time.time()
            > self._topup_bopswarmuptime * 2
        ):
            self._handle_bops_standby_state(standby)

        # BO RF
        if (
            self._topup_borfstandbyenbl
            and self._topup_next - _time.time()
            > self._topup_borfwarmuptime * 2
        ):
    ...
    def _handle_aspu_standby_state(self, state):
        ...
        self._logger.info("Setting PU Voltage to %.1f%...", factor * 100)
        for idx, dev in enumerate(self._pu_devs):
            if not dev.connected:
                self._logger.warning("%s disconnected.", dev.devname)
                continue
    ...
    def _handle_liti_warmup_state(self, state):
        ...
        self._logger.info("LI timing configured.")
    ...
    def _handle_bops_standby_state(self, state):
        ...
        self._logger.info("BO PS timing configured.")
    ...
    def _handle_borf_standby_state(self, state):
        ...
        self._logger.info("BO RF timing configured.")
    ...
    def _update_injstatus(self):
        ...
        while not self.quit:
            ...
            val = (
                1
                if not self._evg_dev.connected
                else self._evg_dev.continuous_state != 1
            )
            ...
            if self._mode == _Const.InjMode.Decay:
                val = (
                    1
                    if not self._evg_dev.connected
                    else self._evg_dev.bucketlist_sync != 1
                )
            ...
            if self.egun_dev.connected:
                ...
                volt = (
                    self._sglbunbiasvolt
                    if self._type == _Const.InjType.SingleBunch
                    else self._multbunbiasvolt
                )
                val = (
                    abs(self.egun_dev.bias.voltage_mon - volt)
            ...
            val = (
                1
                if not self._injsys_dev.connected
                else not self._injsys_dev.is_on
            )
