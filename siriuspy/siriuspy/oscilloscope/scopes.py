...
class Scope:
    ...
    def __init__(
        ...
        Args:
            ...
            channel4 (str, optional): Name of the signal connected to
                ...
                provided. Defaults to None.
            stats (tuple, optional): Measurement statistics to be
    ...
    def _process_meas(self, meas):
        ...
        for stat_split in stat_splits:
            ...
            dct = {fld: val for fld, val in zip(flds, stats)}  # ruff: ignore[zip-without-explicit-strict]
...
class Keysight(Scope):
    ...
    def read_meas(self):
        ...
        meas = self._send_command(b":MEASure:RESults?\n")
        if meas:
    ...
    def read_channel_scales(self, channel):
        ...
        xinc = self._send_command(b":WAVeform:XINCrement?\n")
        yinc = self._send_command(b":WAVeform:YINCrement?\n")
        yor = self._send_command(b":WAVeform:YORigin?\n")
    ...
    def update_channel_scales(self, channel, update=False):
    ...
    def config_acq_wfm(self):
        ...
        idn = self._send_command(b'*IDN?\r\n')  # needed ?
        self._send_command(b":WAVeform:FORMat WORD\n", get_res=False)
        self._send_command(b":WAVeform:BYTeorder MSBF\n", get_res=False)
        return idn
    ...
    def _send_command(self, cmd, get_res=True):
        """."""
        if self._socket:
