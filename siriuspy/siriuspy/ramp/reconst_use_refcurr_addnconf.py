...
def run():
    ...
    new_ntimes = sorted(
        [12.500, 16.000, 19.500, 26.000, 31.000, 36.000, 39.500,
         43.000, 70.000, 100.000, 150.540, 171.500, 192.675, 216.750,
         229.000, 240.500, 280.000, 290.000, 326.500])
    for new_t in new_ntimes:
        strt = '{:.3f}'.format(new_t)
        nconf_strg[strt] = dict()
        nconf_strg[strt]['label'] = 'interp_'+strt.replace('.', 'p')
        for psn in PSNAMES:
            if psn in ('BO-Fam:PS-B-1', 'BO-Fam:PS-B-2'):
                nconf_strg[strt][psn] = R_ORIG.ps_waveform_interp_energy(new_t)
            else:
                mag = get_magnet(MASearch.conv_psname_2_psmaname(psn))
                nr_pts = R_ORIG.ps_ramp_wfm_nrpoints_fams if 'Fam' in psn \
                    else R_ORIG.ps_ramp_wfm_nrpoints_corrs
                wfm_vals = ps2wfm[psn]
                wfm_tims = _np.linspace(0.0, R_ORIG.ps_ramp_duration, nr_pts)
                curr = _np.interp(new_t, wfm_tims, wfm_vals)
                energy = R_ORIG.ps_waveform_interp_energy(new_t)
                strg = mag.conv_current_2_strength(
                    curr, strengths_dipole=energy)
                nconf_strg[strt][psn] = strg
    ...
    r_built = BoosterRamp('testing_newexc5_norm_newconfigs')
    attrs = [
