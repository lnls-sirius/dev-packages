...
from threading import Event as _Event, Lock
...
class EpicsOrbit(BaseOrbit):
    ...
    def set_mturnmask(self, val, beg=True):
        ...
        omsk = bpms[0].tbt_mask_begin if not beg else bpms[0].tbt_mask_end
        omsk = omsk or 0
    ...
    def sync_bpms(self, *args):
        ...
        if self._thread_sync is not None and self._thread_sync.is_alive():
            msg = "WARN: Previous sync. of BPMs still running."
            self._update_log(msg)
            _log.warning(msg)
            return False

        self._thread_sync = _Thread(target=self._synchronize_bpms, daemon=True)
    ...
    def _update_multiturn_orbit_pvs(self):
        for pln, orb in self.smooth_mtorb.items():
            ...
            self.run_callbacks("MTurnIdx" + name + "-Mon", orb[idx, :].ravel())
            if pln == "Sum":
                continue
            nrb = self._csorb.nr_bpms
