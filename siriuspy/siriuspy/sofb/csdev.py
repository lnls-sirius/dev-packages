...
class ConstTLines(_csdev.Const):
    ...
    ApplyDeltaMon = _csdev.Const.register("ApplyDeltaMon", _et.APPLY_DELTA_MON)
...
class SOFBTLines(ConstTLines):
    ...
    def __init__(self, acc):
        ...
        self.cv_pos = _MASearch.get_mapositions(
            map(
                lambda x: x.substitute(dis="MA" if x.dis == "PS" else "PM"),
                self.cv_names,
            )
    ...
    def get_corrs_database(self, prefix=""):
        ...
        dbase = {
            ...
            "CHNickName-Cte": {
                "type": "string",
                "unit": "shortname for the chs.",
                "count": self.nr_ch,
                "value": self.ch_nicknames,
            },
            "CVNickName-Cte": {
                "type": "string",
                "unit": "shortname for the cvs.",
                "count": self.nr_cv,
                "value": self.cv_nicknames,
            },
            "CorrStatus-Mon": {"type": "int", "value": 0b1111111},
            "CorrStatusLabels-Cte": {
                "type": "string",
                "count": len(self.StsLblsCorr._fields),
        ...
        dbase.update(
            {
                ...
                "BPMNickName-Cte": {
                    "type": "string",
                    "unit": "shortname for the bpms.",
                    "count": nbpm,
                    "value": self.bpm_nicknames,
                },
                "OrbStatus-Mon": {"type": "int", "value": 0b00000000},
                "OrbStatusLabels-Cte": {
                    "type": "string",
                    "count": len(self.StsLblsOrb._fields),
...
class SOFBRings(SOFBTLines, ConstRings):
    ...
    def get_orbit_database(self, prefix=""):
        """Return Orbit database."""
        nbpm = self.nr_bpms
        pvs_ring = ["MTurnIdxOrbX-Mon", "MTurnIdxOrbY-Mon", "MTurnIdxSum-Mon"]
        db_ring = dict()
        prop = {
            "type": "float",
            "unit": "um",
        ...
        db_ring.update(
            {
                "SOFBMode-Sel": {
                    "type": "enum",
                    "unit": "SlowOrb_MultiTurn_SinglePass",
                    "value": 0,
