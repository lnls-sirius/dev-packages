...
from .controller import ControllerEpics as \
    _ControllerEpics
...
class PowerSupply(_PSData):
    ...
    def _controller_init(self, current_std):
        if self._controller is None:
            ...
            propty_db = self.propty_database
            self._pwrstate_sel = propty_db['PwrState-Sel']['value']
            if 'OpModel-Sel' in propty_db:
                self._opmode_sel   = propty_db['OpMode-Sel']['value']
            else:
                self._opmode_sel = 0
            if 'Current-SP' in propty_db:
                self._current_sp   = propty_db['Current-SP']['value']
            else:
                self._current_sp   = propty_db['Voltage-SP']['value']
            self._wfmlabel_sp  = self._controller.wfmlabel
            self._wfmload_sel  = 0
            if 'Wfm-SP' in propty_db:
                self._wfmdata_sp   = propty_db['Wfm-SP']['value']
            else:
                self._wfmdata_sp = [0, ]
    ...
    def _get_database(self):
        ...
        if 'Current-SP' in db:
            ...
            db['Voltage-SP']['value']     = self.current_sp
            db['Voltage-RB']['value']     = self.current_rb
            db['VoltageRef-Mon']['value'] = self.currentref_mon
            db['Voltage-Mon']['value']    = self.current_mon
