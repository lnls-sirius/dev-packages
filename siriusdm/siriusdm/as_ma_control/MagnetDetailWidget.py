...
class MagnetDetailWidget(QWidget):
    ...
    def _setWidgetLayout(self):
        ...
        layout.addWidget(QLabel("<h3>" + self._magnet_name + "</h3>"), 0, 0, 1, 3)
    ...
    def _interlockLayout(self):
        layout = QVBoxLayout()
        for i in range(16):
            led = PyDMLed(self, "ca://VAG-"+ self._ps_name + ":Interlock-SP", i)
    ...
    def _metricLayout(self):
        ...
        self.metric_rb_val = PyDMLabel(self, \
                "ca://" + self._magnet_name + ":" + self._metric + "-RB")
        self.metric_rb_val.setPrecision(3)

        self.metric_sp_val = PyDMLineEdit(self, \
    ...
    def _getElementType(self):
        ...
        slow_corrector = re.compile("(SI|BO|LI|TS|TB)-(Fam|\w{2,4}):MA-(CH|CV)(-|\w)*")
        fast_corrector = re.compile("(SI|BO|LI|TS|TB)-(Fam|\w{2,4}):MA-(FCH|FCV)(-|\w)*")
        skew_quad = re.compile("(SI|BO|LI|TS|TB)-(Fam|\w{2,4}):MA-QS")

        if dipole.match(self._magnet_name):
            return "b"
        elif quadrupole.match(self._magnet_name) or skew_quad.match(self._magnet_name):
            return "q"
