...
class ConfigModel(QAbstractTableModel):
    ...
    def setData(self, index, value, role=Qt.EditRole):
        ...
        if index.isValid() and 0 <= row < len(self._vertical_header):
            ...
            if len(self._undo) > self.UNDO_MEMORY:
                self._undo.pop(0)
            self._undo.append((config_name, lambda: self.setDataAlt(row, config_name, cast_val, cast_old_val)))
            #Update Value
    ...
    def _setVerticalHeader(self):
        ...
        self._vertical_header.sort(key=lambda x: elem(x['name']) + subSection(x['name']))

    def _addConfiguration(self, config_name, column, values):
        ''' Adds new configuration to table '''
        #Create new configuration
        if values is None:
            new_configuration = Configuration.getConfiguration(self._config_type, config_name)
            ...
            new_configuration = Configuration(self._config_type, config_name, values)
    ...
    def setDataAlt(self, row, config_name, value, old_value, redo=True):
        ...
        if redo:
            if len(self._redo) > self.UNDO_MEMORY:
                self._redo.pop(0)
            self._redo.append((config_name, lambda: self.setDataAlt(row, config_name, old_value, value, False)))
        else:
            if len(self._undo) > self.UNDO_MEMORY:
                self._undo.pop(0)
            self._undo.append((config_name, lambda: self.setDataAlt(row, config_name, old_value, value, True)))
        #Update view
